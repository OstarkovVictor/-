...
def clicked_voice():
    ...
    audio = pyaudio.PyAudio()
    global streamout
    streamout = audio.open(format=FORMAT, channels=CHANNELS, rate=RATE, input=True, frames_per_buffer=CHUNK, stream_callback=send_voice)
    global streamin
    streamin = audio.open(format=FORMAT, channels=CHANNELS, rate=RATE, output=True, frames_per_buffer=CHUNK)
